(() => {
  ...
  style.textContent = `
    ...
    .littleapps-feature{margin-top:34px;padding:clamp(24px,4vw,42px);border:1px solid #4f5b90;border-radius:24px;background:radial-gradient(circle at 88% 10%,rgba(116,106,255,.24),transparent 32%),linear-gradient(145deg,#151b32,#0d1721);box-shadow:0 24px 90px rgba(0,0,0,.22);overflow:hidden}.littleapps-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(330px,.95fr);gap:34px;align-items:center}.littleapps-feature h2{margin:8px 0 12px;font-size:clamp(2rem,4.5vw,4rem);line-height:.98}.littleapps-feature h2 span{color:#a9a2ff}.littleapps-lead{margin:0;color:#ced7e7;font-size:1.02rem;line-height:1.72}.littleapps-steps{display:grid;grid-template-columns:repeat(6,1fr);gap:7px;margin:20px 0}.littleapps-steps span{padding:9px 6px;text-align:center;border:1px solid #465078;border-radius:9px;background:rgba(22,29,52,.7);color:#e7e9ff;font-size:.68rem;font-weight:850}.littleapps-actions{display:flex;gap:11px;flex-wrap:wrap;margin-top:20px}.littleapps-actions a{text-decoration:none;font-weight:900}.littleapps-actions .primary{padding:12px 16px;border-radius:11px;background:#8d84ff;color:#0e1230}.littleapps-actions .secondary{padding:11px 14px;border:1px solid #59658f;border-radius:11px;color:#e3e8ff}.littleapps-preview{border:1px solid #465078;border-radius:18px;background:#f8fafb;color:#17312b;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.32)}.littleapps-preview-head{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid #dfe6e3;background:#fff}.littleapps-preview-head strong{font-size:.82rem}.littleapps-preview-head span{font-size:.68rem;color:#6c7e79}.littleapps-preview-body{display:grid;grid-template-columns:100px 1fr;min-height:250px}.littleapps-preview-nav{padding:14px 10px;background:#13251f;color:#edf8f5}.littleapps-preview-nav b{display:block;font-size:.72rem;margin-bottom:16px}.littleapps-preview-nav span{display:block;padding:8px;border-radius:7px;background:rgba(255,255,255,.08);font-size:.63rem}.littleapps-preview-main{padding:16px}.littleapps-preview-main h4{margin:0 0 4px}.littleapps-preview-main p{margin:0 0 13px;color:#70827c;font-size:.67rem}.littleapps-cards{display:grid;grid-template-columns:1fr 1fr;gap:7px}.littleapps-card{padding:10px;border:1px solid #dce6e2;border-radius:9px;background:#fff}.littleapps-card strong{display:block;font-size:.68rem}.littleapps-card small{display:block;color:#788983;margin-top:4px;font-size:.58rem}
    ...
    @media(max-width:900px){.modern-features-head,.modern-showcase,.excel-addin-grid,.littleapps-grid{grid-template-columns:1fr}.modern-grid{grid-template-columns:1fr 1fr}.modern-usecases{grid-template-columns:1fr}.excel-demo,.littleapps-preview{max-width:680px}}
    @media(max-width:560px){.modern-features{padding:0 14px}.modern-grid{grid-template-columns:1fr}.modern-card{min-height:auto}.excel-addin,.littleapps-feature{border-radius:18px}.excel-checks{grid-template-columns:1fr}.excel-flow{grid-template-columns:1fr 1fr}.excel-actions a,.littleapps-actions a{width:100%;text-align:center}.littleapps-steps{grid-template-columns:1fr 1fr}.littleapps-preview-body{grid-template-columns:78px 1fr}}
  `;
  ...
  const copy = {
    es: {
      ...
      appsEyebrow:'NUEVO · LITTLEAPPS',
      appsTitle:'De tu API a una <span>aplicación lista para usar.</span>',
      appsLead:'Crea apps web desde Google Sheets con navegación lateral en escritorio e inferior en móvil. Usa páginas personalizadas, hojas distintas por pantalla y gráficos multiserie para comparar personas, fechas o categorías.',
      appsSteps:['General','Datos','Formulario','Panel','Chat','Publicar'],
      appsPrimary:'Crear una app',
      appsDocs:'Ver guía de LittleApps →',
      appsPreviewTitle:'LittleApps Builder',
      appsPreviewState:'Vista previa en vivo',
    ...
    en: {
      ...
      appsEyebrow:'NEW · LITTLEAPPS',
      appsTitle:'From your API to an <span>app ready to use.</span>',
      appsLead:'Build web apps from Google Sheets with desktop side navigation and a mobile bottom bar. Add custom pages, bind screens to different sheets, and create multi-series charts across dates or categories.',
      appsSteps:['General','Data','Form','Dashboard','Chat','Publish'],
      appsPrimary:'Create an app',
      appsDocs:'Read the LittleApps guide →',
      appsPreviewTitle:'LittleApps Builder',
      appsPreviewState:'Live preview',
  ...
  function render() {
    ...
    section.innerHTML = `
      ...
      <section class="littleapps-feature" id="littleapps">
        <div class="littleapps-grid">
          <div>
            <p class="eyebrow">${t.appsEyebrow}</p>
            <h2>${t.appsTitle}</h2>
            <p class="littleapps-lead">${t.appsLead}</p>
            <div class="littleapps-steps">${t.appsSteps.map(item=>`<span>${item}</span>`).join('')}</div>
            <div class="littleapps-actions">
              <a class="primary" href="app.html">${t.appsPrimary}</a>
              <a class="secondary" href="docs/littleapps.html">${t.appsDocs}</a>
            </div>
          </div>
          <div class="littleapps-preview">
            <div class="littleapps-preview-head"><strong>${t.appsPreviewTitle}</strong><span>● ${t.appsPreviewState}</span></div>
            <div class="littleapps-preview-body">
              <aside class="littleapps-preview-nav"><b>LittleApp</b><span>▦ ${t.appsSteps[1]}</span></aside>
              <main class="littleapps-preview-main"><h4>Customers</h4><p>Google Sheets · LittleAPI</p><div class="littleapps-cards"><div class="littleapps-card"><strong>Andrea</strong><small>Active · Quito</small></div><div class="littleapps-card"><strong>Daniel</strong><small>Pending · Manta</small></div><div class="littleapps-card"><strong>Lucía</strong><small>Active · Cuenca</small></div><div class="littleapps-card"><strong>Carlos</strong><small>Active · Loja</small></div></div></main>
            </div>
          </div>
        </div>
      </section>
