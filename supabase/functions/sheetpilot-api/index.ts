...
async function publicLittleApp(request: Request, path: string[]) {
  ...
  if (path[1] === "manifest.webmanifest" && request.method === "GET") {
    const startUrl = `https://littleapi.online/littleapp-v3.html?v=20260919-18&app=${encodeURIComponent(app.slug)}`;
  ...
  return response({
    ...
    app_url: `https://littleapi.online/littleapp-v3.html?v=20260919-18&app=${encodeURIComponent(app.slug)}`,
  }, 200, { "Cache-Control": "no-store" });
}
