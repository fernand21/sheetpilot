...
async function readOperation(api: ApiRecord, request: Request, path: string[]) {
  ...
  if (path[0] === "name") return response({ name: api.name, api_id: api.api_id }); if (path[0] === "metadata" || path[0] === "openapi.json") return metadataResponse(api, path[0] === "openapi.json");
...
function metadataResponse(api: ApiRecord, openapi: boolean) { if (!openapi) return response({ api_id: api.api_id, name: api.name, resource_type: api.resource_type, spreadsheet_id: api.spreadsheet_id, default_sheet: api.default_sheet, permissions: api.permissions, public_read: api.public_read }); return response({ openapi: "3.0.3", info: { title: api.name, version: "1.0.0" }, servers: [{ url: `${supabaseUrl}/functions/v1/sheetpilot-api/${api.api_id}` }], paths: { "/": { get: { summary: "Lista filas" }, post: { summary: "Inserta filas" }, patch: { summary: "Actualiza filas" }, delete: { summary: "Elimina filas" } }, "/search": { get: { summary: "Filtra filas" } }, "/sheets": { post: { summary: "Crea una pestaña" } }, "/format": { post: { summary: "Aplica formato" } }, "/batch": { post: { summary: "Ejecuta cambios atómicos" } } } }); }
